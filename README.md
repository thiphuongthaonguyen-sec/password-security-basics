...
Hier ist deine aktualisierte README.md, die alle Lösungen für die Probleme enthält, die wir gemeinsam gelöst haben (DNS-Fix, TCP-Verbindung und der Wechsel von der defekten PowerShell zu WSL).

Ich habe den Text so strukturiert, dass er sowohl die Sicherheitsaspekte deines Projekts als auch die technischen Hürden bei der Einrichtung professionell dokumentiert.
...
### 3. Docker Daemon Connection
If the error cannot connect to unix:///var/run/docker.sock occurs:
...
## Running with Docker
This project can be executed inside a Docker container to provide a consistent and isolated runtime environment:

### 1. Build the Docker image
```bash
docker build -t secure-data-pipeline .
```

### 2. Run the container
```bash
docker run --rm secure-data-pipeline
```

This container is configured to run as a non-root user in order to follow Docker security best practices and reduce potential attack impact.
